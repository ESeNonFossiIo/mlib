#ifndef _NUMERIX_LICENSE_HANDLER_H__
#define _NUMERIX_LICENSE_HANDLER_H__
...
public:
    LicenseHandler(const std::string& filename_, const CIPHER& cipher_)
        : filename(filename_), cipher(cipher_)
    {
    ...
    void read_license_raw(const std::string& license_file) const
    {
        license = get_file_content(license_file);
        return;
    }

    void generate_raw_license() const
    {
        std::ofstream myfile;
        myfile.open(filename);
        myfile << license;
...
#endif //_NUMERIX_LICENSE_HANDLER_H__
