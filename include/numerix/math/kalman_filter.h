#ifndef NUMERIX_MATH_KALMAN_FILTER_H
#define NUMERIX_MATH_KALMAN_FILTER_H
...
//
// \note All matrices use numerix::Matrixd. State and measurement vectors are
//       passed as single-column matrices (n x 1 and m x 1 respectively).
//
// \code
//   // Constant-velocity 1-D tracker: state = [position, velocity].
//   const double dt = 1.0;
//   numerix::Matrixd F{{1.0, dt}, {0.0, 1.0}};   // x += v·dt
//   numerix::Matrixd H{{1.0, 0.0}};              // we only measure position
//   numerix::Matrixd Q{{1e-3, 0.0}, {0.0, 1e-3}};
//   numerix::Matrixd R{{0.1}};
//   numerix::Matrixd x0{{0.0}, {0.0}};
//   numerix::Matrixd P0{{1.0, 0.0}, {0.0, 1.0}};
//
//   numerix::KalmanFilter kf(F, H, Q, R, x0, P0);
//   kf.predict();
//   kf.update(numerix::Matrixd{{1.2}});          // a position reading of 1.2
//   const numerix::Matrixd& estimate = kf.state();
// \endcode
class KalmanFilter {
public:
    // ------------------------------------------------------------------------
    ...
    KalmanFilter(const Matrixd& F,  ///< [in] state-transition matrix (n x n)
    ...
    void predict(const Matrixd& B, ///< [in] control matrix (n x l)
...
/** @}*/
#endif // NUMERIX_MATH_KALMAN_FILTER_H
