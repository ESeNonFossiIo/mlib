#ifndef NUMERIX_MATH_STATISTICS_NORM_DIST_H
#define NUMERIX_MATH_STATISTICS_NORM_DIST_H
...
{
    return numerix::constants::inv_sqrt_two_pi * std::exp(-0.5 * x * x);
}
...
// \return Φ(x) in (0, 1)
inline double norm_cdf(const double x ///< [in] evaluation point
)
{
    ...
    const double cdf = 1.0 - (numerix::constants::inv_sqrt_two_pi * std::exp(-0.5 * z * z) * k_sum);
...
#endif // NUMERIX_MATH_STATISTICS_NORM_DIST_H
