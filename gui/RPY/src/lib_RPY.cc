...
MainWindow::MainWindow(QWidget* parent) : QMainWindow(parent), ui(new Ui::MainWindow)
{
    ...
    rotation_type = numerix::RotationType::XYZ;
    angle_type = numerix::AngleType::deg;
...
{
    numerix::Angle angle_roll(ui->roll->toPlainText().toDouble(), angle_type);
    numerix::Angle angle_pitch(ui->pitch->toPlainText().toDouble(), angle_type);
    numerix::Angle angle_yaw(ui->heading->toPlainText().toDouble(), angle_type);
    numerix::TaitBryanAngles angles(angle_roll, angle_pitch, angle_yaw);
    numerix::RotationMatrix RM(angles, rotation_type);
...
void MainWindow::on_compute_angles_clicked()
{
    ...
    if (std::abs(det - 1) < 1e-10) {
        numerix::TaitBryanAngles angles = get_roll_pitch_yaw(m, rotation_type, default_yaw);
...
{
    rotation_type = checked ? numerix::RotationType::ZYX : numerix::RotationType::XYZ;
    ui->HPR->setChecked(!checked);
}

void MainWindow::on_HPR_clicked(bool checked)
{
    rotation_type = checked ? numerix::RotationType::XYZ : numerix::RotationType::ZYX;
    ui->RPH->setChecked(!checked);
}

void MainWindow::on_deg_clicked(bool checked)
{
    angle_type = checked ? numerix::AngleType::deg : numerix::AngleType::rad;
    ui->rad->setChecked(!checked);
}

void MainWindow::on_rad_clicked(bool checked)
{
    angle_type = checked ? numerix::AngleType::rad : numerix::AngleType::deg;
