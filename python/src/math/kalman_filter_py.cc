...
/// ----------------------------------------------------------------------------
// Build an mlib::Matrixd from a row-major flat buffer coming from ctypes.
mlib::Matrixd to_matrix(const double* data, const MLIBInt rows, const MLIBInt cols)
{
    return mlib::Matrixd(static_cast<std::size_t>(rows), static_cast<std::size_t>(cols), data);
}

/// ----------------------------------------------------------------------------
// Copy an mlib::Matrixd back into a row-major flat output buffer.
void from_matrix(const mlib::Matrixd& m, double* out)
{
...
MLIB_EXPORT MLIBStatus MLIB_KalmanStep(
