...
def loadNUMERIX():
    ...
    if not hasattr(loadNUMERIX, "numerix"):
        ...
        for candidate in [
            "lib_numerix_bindings_d." + lib_ext,
            "lib_numerix_bindings." + lib_ext,
        ]:
            if isfile(join(lib_dir, candidate)):
                numerix_lib = candidate
                break
        else:
            numerix_lib = "lib_numerix_bindings." + lib_ext

        # get the library
        loadNUMERIX.numerix = cdll.LoadLibrary(join(lib_dir, numerix_lib))

    return loadNUMERIX.numerix
...
def evaluateFunction(name, arg_types, arg_values, ret_type=None):
    ...
    fn = getattr(loadNUMERIX(), "NUMERIX_" + name)
