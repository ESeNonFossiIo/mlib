...
int main()
{
    ...
    std::cout << "check .... " << (M - (R * P * Y)).l_inf_norm() << std::endl;
    std::cout << "check .... " << (N - (Y * P * R)).l_inf_norm() << std::endl;

    std::cout << "check .... " << (M).get_angles().roll(AngleType::deg) << std::endl;
    std::cout << "check .... " << (N).get_angles(RotationType::ZYX).roll(AngleType::deg)
              << std::endl;
}
