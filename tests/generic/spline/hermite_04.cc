...
int main()
{
    ...
    for (std::size_t i = 0; i < i_end; i++) {
        ...
        std::cout << std::setw(15) << convert_vector_to_angle(hs.v(t), ax, clock_wise).deg();
        std::cout << std::setw(15)
                  << convert_vector_to_angle(hs.p(qt) - hs.p(pt), ax, clock_wise).deg();
        ...
        std::cout << std::setw(15) << convert_vector_to_angle(tli(t), ax, clock_wise).deg()
                  << std::endl;
    }
