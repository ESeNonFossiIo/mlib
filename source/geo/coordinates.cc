...
double arclen_of_a_meridian(const double& lat, const ModelConstants& model)
{
    ...
    return a * (lat + (b * sin(2.0 * lat)) + (c * sin(4.0 * lat)) + (d * sin(6.0 * lat)) +
                (e * sin(8.0 * lat)));
}
...
double footpoint_lat(const double& y, const ModelConstants& model)
{
    double sum = model.sm_a + model.sm_b;
    double diff = model.sm_a - model.sm_b;
    ...
    return ya + (b * sin(2.0 * ya)) + (c * sin(4.0 * ya)) + (d * sin(6.0 * ya)) +
           (e * sin(8.0 * ya));
}

void latlon_to_xy(
    const double& lat, const double& lon, const double& lon0, Point& p, const ModelConstants& model)
{
    ...
    p[0] = N * cl * lo + ((N / 6.0) * cl3 * l3 * lo3) + ((N / 120.0) * cl5 * l5 * lo5) +
           ((N / 5040.0) * cl7 * l7 * lo7);

    p[1] = arclen_of_a_meridian(lat) + (t / 2.0 * N * cl2 * lo2) + (t / 24.0 * N * cl4 * l4 * lo4) +
           (t / 720.0 * N * cl6 * l6 * lo6) + (t / 40320.0 * N * cl8 * l8 * lo8);

    return;
}

void xy_to_latlon(
    const Point& p, const double& lon0, double& lat, double& lon, const ModelConstants& model)
{
    double sm_a2 = model.sm_a * model.sm_a;
    double sm_b2 = model.sm_b * model.sm_b;
    ...
    double x4_p = 5.0 + 3.0 * ty2 + 6.0 * nuy2 - 6.0 * ty2 * nuy2 - 3.0 * (nuy2 * nuy2) -
                  9.0 * ty2 * (nuy2 * nuy2);
    double x5_p = 5.0 + 28.0 * ty2 + 24.0 * ty4 + 6.0 * nuy2 + 8.0 * ty2 * nuy2;
    double x6_p = -61.0 - 90.0 * ty2 - 45.0 * ty4 - 107.0 * nuy2 + 162.0 * ty2 * nuy2;
    ...
    /* Calculate latitude */
    lat = lat_y + x2 * x2_p * x_2 + x4 * x4_p * x_4 + x6 * x6_p * x_6 + x8 * x8_p * x_8;

    lat = Angle(lat, AngleType::rad).deg();

    /* Calculate longitude */
    lon = lon0 + x1 * x + x3 * x3_p * x_3 + x5 * x5_p * x_5 + x7 * x7_p * x_7;
...
double compute_convergence_angle(const double& lat, const double& lon)
{
...
Point UTM_latlon_to_xy(const double& lat, const double& lon, const ModelConstants& model)
{
    int zone = compute_zone(lon);

    Point p({0.0, 0.0});

    latlon_to_xy(Angle(lat, AngleType::deg).rad(),
                 Angle(lon, AngleType::deg).rad(),
