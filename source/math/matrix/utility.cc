...
void swap_lines(Matrix<double>& M, Point& b, const std::size_t& l1, const std::size_t& l2)
{
    swap_lines(M, l1, l2);
    double tmp_b = b[l1];
    b[l1] = b[l2];
...
std::pair<Matrix<double>, Point> upper_triangular(const Matrix<double>& M, const Point& b)
{
...
Point solve_upper_triangular(const Matrix<double>& M, const Point& b)
{
    assert(M.r() == b.dim());
    assert(M.c() == b.dim());
...
Point gauss_elimination_upper(const Matrix<double>& M, const Point& b)
{
    assert(M.r() == b.r());
    assert(M.c() == b.r());
...
double rototranslation_matrix(const std::vector<Point>& p,
...
double rototranslation_matrix_2d(
    const Point& p1, const Point& p2, const Point& q1, const Point& q2, Matrix<double>& M, Point& t)
{
...
double best_affine_transformation(const std::vector<Point>& p,
                                  const std::vector<Point>& q,
                                  Matrix<double>& M,
                                  Point& b)
{
...
RotoTranslationMatrixHandler::RotoTranslationMatrixHandler(const std::vector<Point>& u_,
...
void RotoTranslationMatrixHandler::update_mask(const std::vector<size_t>& b_)
{
    b = b_;
    np = 0;
    for (auto v : b) {
...
double RotoTranslationMatrixHandler::get_error()
{
    return error;
}

size_t RotoTranslationMatrixHandler::get_number_of_points()
{
    return np;
}

Matrix<double>& RotoTranslationMatrixHandler::get_matrix()
{
    return M;
}

Point& RotoTranslationMatrixHandler::get_translation()
{
    return t;
}
} // namespace mlib
