...
void Matrix<T>::range_check(size_t i, size_t j) const
{
    if (rows <= i) {
#ifdef WITH_CPP11
        throw std::range_error(" n rows = " + std::to_string(rows) + " : n row out of range");
#else
    ...
    if (cols <= j) {
#ifdef WITH_CPP11
        throw std::range_error(" n cols = " + std::to_string(cols) + " : n col out of range");
#else
...
template <typename T>
Matrix<T>::Matrix(const Matrix<T>& M) : elements(M.elements), rows(M.rows), cols(M.cols)
{
}

template <typename T>
Matrix<T>::Matrix(std::vector<std::vector<double>>& M)
{
    rows = M.size();
    cols = M[0].size();
...
template <typename T>
Matrix<T>::Matrix(size_t rows, size_t cols, const T* elements)
...
template <typename T>
void Matrix<T>::resize(size_t rows_, size_t cols_, const T* elements_)
{
    rows = rows_;
    cols = cols_;
...
Matrix<T>::Matrix(std::initializer_list<std::initializer_list<T>> list)
    : elements(list.size() * list.begin()->size(), T(0.0)), rows(list.size()),
...
template <typename T>
const T& Matrix<T>::element(size_t i, size_t j) const
{
    range_check(i, j);
    return elements[i * cols + j];
}
...
template <typename T>
T& Matrix<T>::element(size_t i, size_t j)
{
    range_check(i, j);
    return elements[i * cols + j];
}
...
template <typename T>
size_t Matrix<T>::r() const
{
    return rows;
}

template <typename T>
size_t Matrix<T>::c() const
{
    return cols;
}

template <typename T>
Matrix<T> Matrix<T>::c(const size_t& i) const
{
    assert(i < cols);
...
template <typename T>
Matrix<T> Matrix<T>::r(const size_t& i) const
{
    assert(i < rows);
    ...
    for (std::size_t i = 0; i < rows; ++i)
        for (std::size_t j = 0; j < M.c(); ++j)
            for (std::size_t k = 0; k < cols; ++k)
                output.elements[i * M.c() + j] +=
                    this->elements[i * cols + k] * M.elements[k * M.c() + j];
    return output;
}
...
template <typename T>
T Matrix<T>::l_p_norm(const std::size_t& p) const
{
    T sum(0.0);
    for (auto it = this->elements.begin(); it != this->elements.end(); ++it)
...
template <typename T>
Matrix<T> Matrix<T>::cofactor(size_t i, size_t j) const
{
...
////////////////////////////////////////////////////////////////////////////////
template <typename S>
std::ostream& operator<<(std::ostream& output, const Matrix<S>& M)
{
...
template <typename S>
Matrix<S> operator*(const double& a, const Matrix<S>& M)
{
    return M * a;
}
...
// identityMatrix
////////////////////////////////////////////////////////////////////////////////
IdentityMatrix::IdentityMatrix(size_t size) : Matrix<double>(size, size)
{
