...
Rotation2DMatrix::Rotation2DMatrix(Angle theta_) : Matrix<double>(2, 2), theta(theta_)
{
...
RotationMatrix::RotationMatrix(const Point& rodriguez_vector_) : Matrix<double>(3, 3)
{
    ...
    if (theta != 0) {
        ...
        Matrixd A({{0, -u[2], u[1]}, {u[2], 0, -u[0]}, {-u[1], u[0], 0}});

        M = std::cos(theta) * I + (1 - std::cos(theta)) * (u * u.t()) + std::sin(theta) * A;
...
RotationMatrix::RotationMatrix(Point axis_, Angle theta_)
    : Matrix<double>(3, 3), axis(axis_), theta(theta_)
{
    Matrix<double> M(
        {{0, -axis.z(), axis.y()}, {axis.z(), 0, -axis.x()}, {-axis.y(), axis.x(), 0}});
    IdentityMatrix id(3);

    Matrix<double> Matrix = std::sin(theta.rad()) * M +
                            std::cos(theta.rad()) * (id - axis * axis.t()) + axis * axis.t();
    for (std::size_t i = 0; i < 9; ++i)
        this->elements[i] = Matrix[i];
}

RotationMatrix::RotationMatrix(Matrix axis_, Angle theta_)
    : Matrix<double>(3, 3), axis({axis_[0], axis_[1], axis_[2]}), theta(theta_)
{
    Matrix<double> M(
        {{0, -axis.z(), axis.y()}, {axis.z(), 0, -axis.x()}, {-axis.y(), axis.x(), 0}});
    IdentityMatrix id(3);
...
RotationMatrix::RotationMatrix(TaitBryanAngles angles, RotationType type)
...
{
    std::pair<Point, Angle> result = get_axis_and_angle(*this);
    axis = result.first;
    theta = result.second;
}

TaitBryanAngles RotationMatrix::get_angles(const RotationType type, const double& defaul_yaw)
{
    return get_roll_pitch_yaw(*this, type, defaul_yaw);
}

TaitBryanAngles
get_roll_pitch_yaw(const Matrixd& m, const RotationType type, const double& defaul_yaw)
{
    Angle roll(0);
    Angle yaw(0);
    Angle pitch(0);
