...
HyperPlane::HyperPlane(std::initializer_list<double> list, bool normal_form) : Point(list)
{
    if (normal_form)
        this->normal_form();
}

void HyperPlane::normal_form()
{
...
HyperPlane middle_hyperplane_between_points(const Point& p, const Point& q)
{
    HyperPlane hp;
    hp.resize(p.dim() + 1);
...
Point get_hyperplanes_intersection(const HyperPlane& p, const HyperPlane& q, const HyperPlane& r)
{
    Matrix<double> m(3, 3);
    Point b;
    b.resize(3);
...
HyperPlane hyperplane_passing_through_three_points(const Point& a, const Point& b, const Point& c)
{
    Point p1(b);
    p1 -= a;
    Point p2(c);
...
Point circumference_center(const Point& a, const Point& b, const Point& c)
{
    if (a.dim() == 3) {
