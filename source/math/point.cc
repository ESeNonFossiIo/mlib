...
Point::Point(std::initializer_list<double> list, bool normalize)
    : Matrix<double>(Matrix<double>({list}).t()), is_normalized(normalize)
{
    if (is_normalized)
        this->normalize();
}

Point::Point(double x_) : Matrix<double>(1, 1)
{
    elements[0] = x_;
}

Point::Point(double x_, double y_) : Point({x_, y_})
{
}

Point::Point(double x_, double y_, double z_) : Point({x_, y_, z_})
{
}

Point::Point(double x_, double y_, double z_, double t_) : Point({x_, y_, z_, t_})
{
}
...
Point::Point(const pcl::PointXYZI& p) : Point(p.x, p.y, p.z)
{
}
...
void Point::resize(const size_t& n)
{
    this->rows = n;
    this->cols = 1;
...
Point Point::operator=(const Matrix<double> M)
{
    this->resize(M.r());
    for (size_t i = 0; i < this->dim(); ++i)
        this->elements[i] = M(i);
...
Point operator^(const Point& p1, const Point& p2)
{
    ...
    return Point(p1.y() * p2.z() - p1.z() * p2.y(),  // a_y b_z - a_z b_y
...
double points_distance(const Point& p1, const Point& p2, const std::size_t& d)
{
    double sum = 0;
