...
MathSeq<T>::MathSeq(const std::vector<T>& init_sequence,
    ...
    : sum(0), sequence(init_sequence), next_element(next_element_)
{
...
template <typename T>
void MathSeq<T>::compute_elements(const int n_elements)
{
    if (static_cast<std::size_t>(n_elements) < sequence.size())
        return;

    sequence.resize(n_elements);
    for (std::size_t i = size; i < static_cast<std::size_t>(n_elements); ++i) {
        auto tmp = std::vector<T>(sequence.begin() + i - pattern_lenght, sequence.begin() + i);
...
template <typename T>
typename std::vector<T>::iterator MathSeq<T>::begin()
{
    return sequence.begin();
}

template <typename T>
typename std::vector<T>::iterator MathSeq<T>::end()
{
    return sequence.end();
}
