...
void Logger::save_on_file(const std::string& filename_)
{
    if (filename_ != "") {
        filename = filename_;
        write_on_file = true;
...
void Logger::write(const std::string& msg, const std::string& str, const GeneralColor& color)
{
    clock_t end = clock();
...
#ifndef WIN32
    size_t size = std::snprintf(nullptr, 0, format, elapsed_secs, msg.c_str(), str.c_str());
#else
    size_t size = _snprintf(nullptr, 0, format, elapsed_secs, msg.c_str(), str.c_str());
#endif
...
void Logger::status(const std::string& str)
{
    write("STATUS", str, blue);
}

void Logger::value(const std::string& str, const std::string& val)
{
    auto format = "%20s = %20s";

#ifndef WIN32
    size_t size = std::snprintf(nullptr, 0, format, str.c_str(), val.c_str());
#else
    size_t size = snprintf(nullptr, 0, format, str.c_str(), val.c_str());
#endif
...
void Logger::msg(const std::string& str)
{
    write("MSG", str, white);
}
