...
XMLEntry process_XML_text(const std::string& str)
{
    ...
    std::size_t end_label = std::min(str.find('>', init + 1), str.find(' ', init + 1));
    ...
    std::string properties_raw = str.substr(init_first_tag + 1, end_first_tag - 1 - init_first_tag);

    std::replace(properties_raw.begin(), properties_raw.end(), '\'', '\"');

    std::vector<std::string> properties = split(properties_raw, " ", false, false);

    for (auto it = properties.begin(); it != properties.end(); it++) {
        std::vector<std::string> entry = split(*it, "=", false, true);
        std::size_t init_prop = entry[1].find("\"") + 1;
        std::size_t end_prop = entry[1].find("\"", init_prop);
        ret.properties.insert(std::pair<std::string, std::string>(
            entry[0], entry[1].substr(init_prop, end_prop - init_prop)));
    }
...
XMLHandler::XMLHandler(std::string filename, bool is_file)
{
    std::string text = "";
    ...
    while ((0 < text.find('<')) && (text.find('<') < text.size())) {
        ...
        if (entry.label == "xml") {
            ...
            for (auto it = entry.properties.begin(); it != entry.properties.end(); ++it) {
                std::string a = (*it).first;
                std::string b = (*it).second;
                xml_stylesheet.insert(
                    std::make_pair<std::string, std::string>(a.c_str(), b.c_str()));
            }
        }
        ...
        if (entry.label != "xml-stylesheet" && entry.label != "xml") {
            for (auto it = entry.properties.begin(); it != entry.properties.end(); ++it) {
                std::string a = (*it).first;
                std::string b = (*it).second;
                properties.insert(std::make_pair<std::string, std::string>(a.c_str(), b.c_str()));
            }
            xml_entries.insert(std::make_pair<std::string, XMLHandler>(
                entry.label.c_str(), XMLHandler(entry.text, false)));
        }
    }
    val_text = text;
}

void XMLHandler::print(int indent)
{
    if (xml.size() > 0) {
        std::cout << "<?xml ";
...
// }
//
XMLHandler XMLHandler::operator[](const std::string& s)
{
    return xml_entries[s];
}

std::string XMLHandler::operator()()
{
