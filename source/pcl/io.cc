...
void save_ply(const std::string& filename, pcl::PointCloud<pcl::PointXYZI>& cloud)
{
              ...
              const std::string& separator)
{
    ...
    for (auto q : cloud) {
        filename_out << q.x << separator << q.y << separator << q.z << separator << q.intensity
                     << std::endl;
    }
    filename_out.close();
    return;
              ...
              const std::string& separator)
{
    ...
    for (auto q : cloud) {
        filename_out << q.x << separator << q.y << separator << q.z << separator << q.r << separator
                     << q.g << separator << q.b << std::endl;
    }
    filename_out.close();
    return;
}
