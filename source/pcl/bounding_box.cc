...
BoundingBox::BoundingBox(pcl::PointCloud<pcl::PointXYZI>::Ptr input_cloud)
    : bounding_box(new pcl::PointCloud<pcl::PointXYZI>),
      projectionTransform(Eigen::Matrix4f::Identity())
{
    update(input_cloud);
...
void BoundingBox::update(pcl::PointCloud<pcl::PointXYZI>::Ptr input_cloud)
{
    ...
    // Transform the original cloud to the origin where the principal components correspond to the
    // axes.

    projectionTransform.block<3, 3>(0, 0) = eigenVectorsPCA.transpose();
    projectionTransform.block<3, 1>(0, 3) =
        -1.f * (projectionTransform.block<3, 3>(0, 0) * centroid.head<3>());

    pcl::transformPointCloud(*original_points, *projected_points, projectionTransform);

    pcl::getMinMax3D(*projected_points, minPoint, maxPoint);

    pcl::PointCloud<pcl::PointXYZI>::Ptr extreme_point(new pcl::PointCloud<pcl::PointXYZI>);
    pcl::PointCloud<pcl::PointXYZI>::Ptr extreme_point_old(new pcl::PointCloud<pcl::PointXYZI>);
    ...
    pcl::transformPointCloud(*extreme_point_old, *bounding_box, projectionTransform_inv);
...
void BoundingBox::points_in_the_bb(pcl::PointCloud<pcl::PointXYZI>::Ptr input_cloud,
                                   ...
                                   bool inside)
{
    ...
    out_cloud->clear();
    pcl::transformPointCloud(*original_points, *transformed_points, projectionTransform);

    bool point_is_inside;
    for (auto q : *transformed_points) {
        if ((q.x >= minPoint.x) && (q.x <= maxPoint.x) && (q.y >= minPoint.y) &&
            (q.y <= maxPoint.y) && (q.z >= minPoint.z) && (q.z <= maxPoint.z)) {
    ...
    pcl::transformPointCloud(*output, *out_cloud, projectionTransform_inv);
}

Segment BoundingBox::get_dominant_direction()
{
    pcl::PointXYZI a, b;
    double max = 0;
    ...
    for (size_t p = 0; p < 3; ++p) {
        size_t pp = pow(2, p);
        pcl::PointCloud<pcl::PointXYZI>::Ptr even(new pcl::PointCloud<pcl::PointXYZI>);
        pcl::PointCloud<pcl::PointXYZI>::Ptr odd(new pcl::PointCloud<pcl::PointXYZI>);
...
std::vector<double> BoundingBox::sizes()
{
    std::vector<double> s;
...
STATUS
BoundingBox::is_flat(double tolerance)
{
